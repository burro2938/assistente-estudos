...
def gerar_flashcards_personalizados(quantidade, materia, dificuldade, ano_aluno, texto_apontamentos=""):
    ...
    # Bloquear a criação de flashcards para TIC, Educação Tecnológica, Educação Visual e Educação Física em qualquer situação
    if any(m in materia_inf for m in ["tic", "tecnologias de informação", "tecnológica", "visual", "física", "fisica"]):
        return []
...
# Barra Lateral de Navegação (5 pontos ajustados)
...
# 2. Calendário
    ...
    if st.session_state.selected_date is not None:
        ...
        # Procurar registos para esta data
        ...
        # Obter matriz do mês (começando à segunda-feira)
        ...
        # Mapear logs por data string para consulta rápida
        logs_por_data = {}
        for log in st.session_state.logs:
...
# 4. Registo Diário
    ...
    if st.session_state.step_registo == "formulario":
        ...
        if not st.session_state.flashcards_pos_gerados:
            st.info("Não há flashcards gerados para as disciplinas registadas hoje (ou a disciplina selecionada não suporta flashcards automáticos).")
        else:
...
# 5. Alínea: Estudar
    ...
    if st.session_state.step_estudar == "escolher_materia":
        ...
        # Renderização dinâmica dos exercícios ou flashcards
        if st.session_state.atividade_selecionada == "Exercícios":
            ...
            if st.button("Submeter e Corrigir Respostas", key="btn_submeter_30"):
                ...
                # Registar automaticamente no calendário ao submeter com o método correto
            ...
            # Verificar se a matéria selecionada é uma das bloqueadas
            mat_atual_inf = st.session_state.materia_escolhida_estudo.lower()
            bloqueado_flashcards = any(m in mat_atual_inf for m in ["tic", "tecnologias de informação", "tecnológica", "visual", "física", "fisica"])

            if bloqueado_flashcards:
                st.warning("⚠️ Não é possível gerar flashcards para esta disciplina (TIC, Educação Tecnológica, Educação Visual ou Educação Física). Por favor, escolhe outra disciplina ou método de estudo.")
            else:
